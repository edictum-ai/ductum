...
| Harness registry and canonical events | decisions/054, decisions/057, decisions/080 | Partially landed. `@ductum/harness` now exports `loadBuiltInHarnessAdapters()` and canonical event helpers for Claude/Codex/Copilot, but OpenCode/historical adapter parity is still outside that registry path. Keep this open until the remaining supported harnesses move behind the same boundary or are explicitly retired. | `harness-registry-canonical-events` |
