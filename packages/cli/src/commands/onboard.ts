  ...
  program
    ...
    .action(createAction(deps, async (ctx, rawPath: string, options: OnboardOptions) => {
      ...
      if (options.dryRun === true) {
        ctx.write({ repoPath, profilePath, profile, ...detection }, renderPlan(repoPath, profilePath, detection, true))
        return
      }
      ...
      ctx.write(
        { project, repoPath, profilePath, stack: detection.stack, verifyCommands: detection.verifyCommands },
        renderPlan(repoPath, profilePath, detection, false),
      )
    }))
}
...
function renderPlan(
  ...
  const suffix = dryRun ? 'dry-run: no files written and no API calls made' : 'next: ductum doctor --json'
  return `${summary}\n${suffix}`
}
