...
function buildBlindReviewPrompt(prompt: string, candidates: Task[], policy: string): string {
  ...
  return [
    ...
    'Your review feedback MUST include one JSON block with this exact ductum-review-result contract:',
    '```json',
    '{',
    ...
    '}',
    '```',
    'Put this JSON block in the `ductum_complete` result. Evidence alone is not enough and cannot override a malformed completion.',
