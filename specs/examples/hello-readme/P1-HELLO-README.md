...
- decisions/060: silent drift is not acceptable; the operator needs a
  real diff, not a success-looking diary note.

## Behavior Contract

- The task must append exactly one new line to `README.md` and must not
  rewrite unrelated README content.
- If the requested line already exists, the run must fail loudly instead
  of pretending a diff happened.
- Verification must preserve an operator-visible README diff so the
  bootstrap proof shows a real repo mutation.
...
- Record a Ductum decision before changing this task away from a
  one-line README proof.

## Slop Review

- Confirm explicit evidence shows the patch is exactly one new README line
