  ...
  return (
    ...
    >
      ...
      <Mono size={10} color={tokens.dim} style={{ marginTop: 6, display: 'block', lineHeight: 1.35 }} title={CLEAN_DONE_TITLE}>
        {metrics.costPerCleanDoneLabel}/clean done · {metrics.weekDeltaLabel}
      </Mono>
    </Link>
  )
}
...
function weekSpendMetrics(runs: EnrichedRun[]) {
  ...
  const weekSpent = totalCost(current)
  const previousSpent = totalCost(previous)
...
function totalCost(runs: EnrichedRun[]): number {
  return runs.reduce((acc, run) => acc + runCost(run).usd, 0)
}
