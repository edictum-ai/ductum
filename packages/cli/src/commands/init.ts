...
export function registerInitCommand(program: Command, deps: CliProgramDeps) {
  program
    ...
    .option('--dir <path>', 'Install directory. Defaults to ~/ductum.')
    .option('--name <projectName>', 'Project name. Defaults to factory.')
    ...
    .option('--no-browser', 'Do not open the browser; print the dashboard URL and manual access command')
