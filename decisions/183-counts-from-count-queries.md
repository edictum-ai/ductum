...
3. A full unbounded list may still be fetched behind an explicit count API when
   the count depends on semantics that are not yet expressible in SQL. The API
   must document why the call is unbounded and must not leak the list method
   into the summary surface.
...
- `packages/api/src/lib/operator-queue-counts.ts` is the explicit count API for
  operator queue run counts whose leaf/follow-up semantics currently live in
  TypeScript.
